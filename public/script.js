...
function productCard(product) {
    ...
    return `<article class="product-card"><div class="product-card-image"><img src="${escapeHtml(image)}" alt="${escapeHtml(imageAlt)}" width="1200" height="900" loading="lazy" decoding="async" data-fallback="${fallback}"></div><div class="product-card-body"><h3><a href="product.html?id=${encodeURIComponent(product.id)}">${escapeHtml(product.name)}</a></h3><p>${escapeHtml(product.description)}</p><div class="product-meta"><span>${escapeHtml(product.type)}</span><span>${escapeHtml(product.abv || "ABV to confirm")}</span></div><div class="product-footer"><div><span class="price">${formatPrice(product.price)}</span><small class="availability ${availabilityClass}">${availability}</small></div><div class="card-actions">${action}</div></div></div></article>`;
}
function setupImageFallbacks(root = document) { root.querySelectorAll("img[data-fallback]").forEach(image => image.addEventListener("error", () => { if (image.dataset.fallback) { image.src = image.dataset.fallback; delete image.dataset.fallback; } }, { once: true })); }
function renderProducts(list, target) { if (target) { target.innerHTML = list.length ? list.map(productCard).join("") : `<div class="empty-state"><h3>No matches yet</h3><p>Try a different name, category, or search term.</p></div>`; setupImageFallbacks(target); } }
